    ...
    <ol className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
      {steps.map((step, i) => (
        <li key={step.title} className="relative rounded-md border border-border bg-white p-6 shadow-sm">
          <span
            className={`flex h-10 w-10 items-center justify-center rounded-full text-body-lg font-heading ${
              onDark ? "bg-gold-500 text-ink-950" : "bg-ink-950 text-gold-300"
            }`}
          >
            {i + 1}
          </span>
          <h3 className="mt-4 text-h4-mobile text-ink-950">{step.title}</h3>
