      ...
      {items.map((item, idx) => {
        ...
        return (
          ...
          >
            ...
            {/* PRO V2.1 §56: was conditionally rendered (instant show/hide, no transition
                possible on an element that isn't there yet). Always rendered now; height comes
                from animating the grid track itself (0fr → 1fr) rather than a guessed pixel
                max-height, so it works for any answer length without measuring anything. */}
            <div
              className="grid transition-[grid-template-rows] duration-[300ms] ease-standard"
            ...
            >
              <div className="overflow-hidden">
                <div
                  ...
                  aria-hidden={!isOpen}
                  className="pl-[52px] pr-5 pb-5 text-body text-text-secondary"
                >
                  {item.answer}
                </div>
              </div>
